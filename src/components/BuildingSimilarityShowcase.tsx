...
type ShowcaseBuilding = SimilarityBuilding & {
 ...
 function getNavigationTarget(building: ShowcaseBuilding) {
  if (building.route !== '/timeline') {
    return building.route;
  }

  return `${building.route}?focus=${encodeURIComponent(building.name)}`;
 }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="gold-border rounded-lg p-5 bg-imperial-deeper/50 lg:col-span-2">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              ...
              <button
                onClick={() => navigate(getNavigationTarget(activeBuilding))}
              ...
              {recommendations.map((item) => {
                ...
                return (
                  <div key={item.building.id} className="rounded-lg border border-imperial-gold/10 bg-black/20 p-4">
                    ...
                    <button
                      onClick={() => navigate(getNavigationTarget(target))}
